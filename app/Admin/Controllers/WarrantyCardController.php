...
class WarrantyCardController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->column('card_id', '保卡編號');
        $grid->column('name', '用戶名');
        $grid->column('mobile', '手機');
        $grid->column('address', '地址');
        $grid->column('email', __('Email'));
        $grid->column('car_license', '車牌');
        $grid->column('car_brand', '車子品牌');
        $grid->column('warranty_type', '保卡型號');
        $grid->column('construction_by', '施工店家號');
        $grid->column('construction_date', '施工日期');
        $grid->column('warranty_body', '施工項目');
        $grid->column('price', '施工費用');
        $grid->column('recommand_user_id', '推薦人ID');
        $grid->column('status', '保卡狀態');
    ...
    protected function form()
    {
        ...
        $form->date('construction_date', '施工日期')->default(date('Y-m-d'));
        $form->textarea('warranty_body', '施工項目');
        $form->text('price', '施工費用');
        $form->text('recommand_user_id', '推薦人ID')->default('Nobody');
        $form->text('status', '保卡狀態')->default('OFF');
